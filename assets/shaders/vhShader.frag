...
void main(){
  ...
  for( float x = -4.0; x < 2.5; x += 1.0 ){
    col.xyz += vec3(
      flixel_texture2D( bitmap, uvn + vec2( x - 0.0, 0.0 ) * 7E-3 ).x,
      flixel_texture2D( bitmap, uvn + vec2( x - 2.0, 0.0 ) * 7E-3 ).y,
      flixel_texture2D( bitmap, uvn + vec2( x - 4.0, 0.0 ) * 7E-3 ).z
    ) * 0.1;
  }
  col *= 0.6;
  ...
  vec4 text = flixel_texture2D(bitmap, openfl_TextureCoordv);
